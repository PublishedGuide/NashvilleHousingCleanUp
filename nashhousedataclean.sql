...
# This information could be populated from the information we can find in the table as reference
# Here we see that there is a relationship between ParcelID and PropertyAddress, where we can tell that ParcelID is indicative of the PropertyAddress
# This means we can fill in empty or Null PropertyAddress column values if there is an assigned ParcelID that already has a related address
SELECT *
FROM PortfolioProject.dbo.NashvilleHousing
ORDER BY ParcelID

#Use a self join to run a conditional statement
#Essentially we want to assign the property address for a parcelID on one unique id, to the same parcel ID on another UniqueID that doesnt already have  a property address value this query below will show us the section of the table where we need to apply these changes and what the changes would be
